...
Initialize link to our dom element with help of [useRef](https://ru.reactjs.org/docs/hooks-reference.html#useref), and now we have a connection with our canvasт, and then through `useEffect` we are getting inner canvas context engine (`ctx`) and draw a rectangle.
...
[Cleanup useeffectt](https://reactjs.org/docs/hooks-effect.html#effects-with-cleanup).
